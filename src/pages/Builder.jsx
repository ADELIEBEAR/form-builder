...
export default function Builder() {
  ...
  // ── 질문 CRUD 및 기능 함수들 (정프로님 원본 100% 유지) ──
  function addQ(type) {
    const q = {
          ...
          <div className={s.lbody}>
            ...
            {activeTab === 1 && (
              <>
                ...
                <div className={s.lsec}>배경 이미지 설정</div>
                <SetRow label="배경 이미지 등록">
                  <label className={s.fileBtn}>
                    {bgImgData ? '✅ 배경 이미지 등록됨' : '🖼️ 배경 업로드'}
                    <input type="file" accept="image/*" style={{ display:'none' }} onChange={onBgImg} />
                  </label>
                  {bgImgData && <button className={s.delImgBtn} onClick={() => setBgImgData(null)}>배경 제거</button>}
                </SetRow>
                <SetRow label="배경 블러 강도">
            ...
            {activeTab === 2 && (
              <>
                ...
                {settings.useStart && (
                  <>
                    <SetRow label="커버 이미지 등록">
                      <label className={s.fileBtn}>
                        {coverImgData ? '✅ 커버 등록됨' : '🖼️ 이미지 업로드'}
